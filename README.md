...
Each Sync Item records its most recent synchronization time in configuration. Run a preview to fetch the Source again and determine the current file-level differences. A no-change preview can still be confirmed: RepoMirror rechecks the Source and Destination, copies nothing, and records the result. A missing Destination appears as a directory-creation change even when the Source has no ordinary files.
