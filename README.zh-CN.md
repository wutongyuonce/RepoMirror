...
3. 点击同步项的“预览并同步”，或点击顶部“同步当前文件夹”。批量同步可选择仅当前节点，或递归包含子文件夹组（默认）。预览列出新增、修改、删除和缺失目标目录的创建；某条预览失败会单独显示，不妨碍查看和确认其他成功项。确认后才写入本地文件。即使文件已一致，也可确认并记录本次同步结果。
4. 每条同步项默认开启镜像模式：确认同步后，目标目录里来源没有的普通文件会被删除。若要保留这些本地额外文件，在同步项详情中关闭“镜像同步”。两种模式都不会删除未列出的空目录；遇到符号链接或文件与目录冲突会报错。
...
同步始终会先展示预览，确认时会重新检查来源提交和文件差异。镜像模式可能删除目标目录中来源不存在的文件；关闭镜像模式则会保留本地额外文件。来源中的符号链接、目标中的符号链接或文件与目录冲突会阻止同步。
...
同步项会在配置中记录最近一次同步时间。运行预览会重新获取来源，并判断当前的文件级差异。预览为空时仍可确认：应用会再次核对来源和目标，不复制文件，只记录本次同步结果。若目标目录不存在，即使来源没有普通文件，预览也会列出创建目录。
