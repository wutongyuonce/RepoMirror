...
- 可将一个或多个同步项拖至另一根目录或文件夹组；仅当被移动项的本地目标文件夹存在时才会询问是否一并移动，本地不存在则只更新配置。拖动完成后保持当前页面不跳转。
- 第一层根目录支持右键“移动至”：选择目标父文件夹后，可把整个本地文件夹移过去，或只改设置并保留原文件夹。
- 按住 Command（或 Control）点击可多选，按住 Shift 点击可选择两个条目之间的全部内容。
- 根目录、文件夹组和同步项的最终文件夹名称都可重命名。若对应本地文件夹存在，可选择是否一并改磁盘上的名字；不存在则只改配置。
...
同步项仍会在配置中记录最近一次同步时间。运行预览会重新获取来源，并判断当前的文件级差异。预览为空表示无需同步，此时 RepoMirror 不会提供确认同步操作。
