...
RepoMirror 导入和导出使用 `schemaVersion: 3` 的 JSON。未知字段会被拒绝。旧版 v2 导入会被拒绝；已保存的 v2 文件会保留，并报告版本不受支持。
...
| `rootDirectories` | array | `id` 唯一，根目录的实际绝对位置不能重叠；校验时会解析已有上级目录的符号链接。 |
...
| --- | --- |
| `id` | 非空且唯一的字符串。 |
| `path` | 绝对本地文件夹路径；导入或读取配置时可以尚不存在，同步时会自动创建。已有的上级路径必须可访问且为目录。 |
| `name` | 可选显示名称。 |
