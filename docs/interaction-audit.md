...
| An already matching Destination remained marked “never synced,” and an empty selected scope did nothing when preview was requested. | A no-change preview can be confirmed to record synchronization; an empty scope reports that it contains no items. |
