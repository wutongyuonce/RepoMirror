...
1. Ask only for unknown essentials: one or more root directory paths, folder group paths, GitHub links, optional destination names, and mirror preference. For an ambiguous `tree` or `blob` URL whose branch may contain `/`, ask for the full branch name instead of guessing.
...
3. Convert each GitHub URL into exactly one Sync Item with that item's `rootId`. Accept only HTTPS `github.com` repository, `tree/<branch>/<path>`, or `blob/<branch>/<path>` directory URLs without credentials, ports, query strings, or fragments. Normalize a repository clone URL ending in `.git` to a plain repository `sourceUrl`. A repository URL uses `null` for `branch` and `sourcePath`; a directory URL sets both fields to the exact branch and remaining directory path. A file path may pass configuration validation but fails at preview.
