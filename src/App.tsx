  ...
  return <main className="app-shell" data-theme={theme} onClick={() => contextMenu && setContextMenu(undefined)} onDragOverCapture={autoScroll}>
    ...
    {dialog === "preview" && <Modal title="同步预览" close={() => setDialog(null)} wide>
      ...
      {(pendingSyncs.length > 0 || previewFailures.length > 0) && <div className="preview-list">
        {previews.flatMap(({ item, preview }) => preview.changes.map((change) => <div key={`${item.id}-${change.path}`}><span className={`change change-${change.kind.split("_")[0]}`}>{change.kind === "add" ? "新增" : change.kind === "modify" ? "修改" : change.kind === "create_directory" ? "创建目录" : change.kind === "add_symlink" ? "新增链接" : change.kind === "modify_symlink" ? "修改链接" : change.kind === "delete_symlink" ? "删除链接" : "删除"}</span><code>{itemPath(item)}{change.path === "." ? "" : `/${change.path}`}{change.linkTarget ? ` → ${change.linkTarget}` : ""}</code></div>))}
